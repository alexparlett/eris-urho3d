...
void CreateCamera()
{
    ...
    SubscribeToEvent("MouseWheel", "ViewMouseWheel");
...
void UpdateView(float timeStep)
{
    if(input.mouseButtonDown[MOUSEB_MIDDLE])
    {
            ...
            else if(mouseMove.x < -2)
        {
            ...
            if(mouseMove.y > 2)
        {
                cameraNode.TranslateRelative(Vector3(0, cameraBaseSpeed, 0) * timeStep * speedMultiplier);
        }
            else if(mouseMove.y < -2)
        {
                cameraNode.TranslateRelative(Vector3(0, -cameraBaseSpeed, 0) * timeStep * speedMultiplier);
        }
        }
    ...
    if (input.mouseButtonDown[MOUSEB_RIGHT])
    {
        ...
        if (mouseMove.x != 0 || mouseMove.y != 0)
        {
            ...
            if (input.keyDown[KEY_LALT] && (selectedNodes.length > 0 || selectedComponents.length > 0))
            {
    ...
    if (orbiting && !input.keyDown[KEY_LALT])
        orbiting = false;
...
void ViewMouseWheel(StringHash eventType, VariantMap& eventData)
{
    if(ui.focusElement is null)
    {
        ...
        int scroll = eventData["Wheel"].GetInt();

        if(scroll > 0)
        {
            cameraNode.TranslateRelative(Vector3(0, 0, cameraBaseSpeed) * speedMultiplier);
        }
        else if(scroll < 0)
        {
            cameraNode.TranslateRelative(Vector3(0, 0, -cameraBaseSpeed) * speedMultiplier);
