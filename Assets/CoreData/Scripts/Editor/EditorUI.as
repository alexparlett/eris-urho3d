...
void CreateUI()
{
    ...
    uiStyle = cache.GetResource("XMLFile", "UI/DebugStyle.xml");
...
void ResizeUI()
{
    ...
    for (uint i = 0; i < children.length; ++i)
    {
        if (children[i].type == WINDOW_TYPE)
            children[i].SendEvent("LayoutUpdated");
    }
}
...
void UpdateWindowTitle()
{
    ...
    if (sceneModified)
        sceneName += "*";
    graphics.windowTitle = "Solarian Wars Editor - " + sceneName;
}
